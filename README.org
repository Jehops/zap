...
** Environment variables
   ...
   =zfs send= and =zfs recv= commands will be piped through the contents of the variable.  Examples of this using =~mbuffer~= are given in the =Examples= section above.

   =ZAP_PREFIX=

   Prefix all =zfs= commands with the contents of the variable.  This is intended to allow privilege escalation with sudo or similar tools, which is a requirement on Linux if you do not want to run zap as root until pool-level access permissions are implemented in OpenZFS—but usage is not limited to this.
