...
** Description
   Create snapshots of ZFS file systems with the specified time to live (TTL).
...
[2] New snapshots are only created when a file system has changed since the last
snapshot.  If the file system hasn't changed, then the timestamp of the newest
snapshot is updated.
