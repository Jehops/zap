...
#+END_SRC
     ...
     Crontab entries [fn:1],[fn:2] for rolling snapshots and remote replication.
...
[fn:1] If you are concerned with <span class="file">/etc/crontab</span> being changed when the OS is upgraded, use <span class="file">crontab -e</span> instead of editing <span class="file">/etc/crontab</span>.
[fn:2] Ensure the location where zap is installed (usually <span class="file">/usr/local/bin</span>) is listed in the PATH declaration at the top of <span class="file">/etc/crontab</span>.  Alternatively, use zap's full path in the crontab entries.
